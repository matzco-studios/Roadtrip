...
namespace UI
{
    ...
    public class ActionMessageController : MonoBehaviour
    {
        ...
        {
            _press.enabled = active;
            _key.enabled = active;
            _action.enabled = active;
            _itemName.enabled = active;
        ...
        {
            InteractableItem(itemName: item.GetComponent<GrabbableItem>()?.Name ?? item.name, action: "to grab",
                key: "E");
        }
